...
The tmux status line modules are set as variables and prefixed with `@everforest_status_<module>`.
...
set -g status-right "#{E:@everforest_status_application}#{E:@everforest_status_session}"
set -g status-left ""
```
...
```sh
set -g @everforest_[module_name]_icon "icon"
```

### Override the specific module color

```sh
set -g @everforest_[module_name]_color "color"
```

### Override the specific module text

```sh
set -g @everforest_[module_name]_text "text"
```

### Override the specific module's background color

```sh
set -g @everforest_status_[module_name]_bg_color "#{@thm_surface_0}"
```

### Removing a specific module option

```sh
set -g @everforest_[module_name]_[option] ""
```
...
```sh
set -g @everforest_date_time_icon ""
```

### Notes for TPM users

Make sure you load the everforest theme prior to setting the status-left and/or
status-* options. This ensures the everforest options (such as colors and
...
run '~/.config/tmux/plugins/tmux/everforest.tmux' # or where this file is located on your machine

# ... and then set status-left & status-right ...
set -g status-left "#{E:@everforest_status_session}"

set -g status-right "#{E:@everforest_status_[module_name]}"
set -ag status-right "#{E:@everforest_status_[module_name]}"
set -agF status-right "#{E:@everforest_status_[module_name]}"
...
set -agF status-right "#{E:@everforest_status_weather}"
...
set -agF status-right "#{E:@everforest_status_clima}"
...
set -agF status-right "#{E:@everforest_status_load}"
```
...
set -agF status-right "#{@everforest_status_gitmux}"
```
...
Add the following to your `~/.gitmux.conf` so that it uses everforest colors:
...
set -agF status-right "#{E:@everforest_status_pomodoro_plus}"
...
```sh
set -g @everforest_kube_context_color "#{@thm_red}"
set -g @everforest_kube_namespace_color "#{@thm_sky}"

run ~/.config/tmux/plugins/everforest/tmux/everforest.tmux

set -agF status-right "#{E:@everforest_status_kube}"
